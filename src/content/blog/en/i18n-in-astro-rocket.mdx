...
Astro Rocket's blog, projects, and pages collections already carry a `locale` field on their schema (`src/content.config.ts`). Translated content lives in a parallel folder structure:
...
In the Dutch post's frontmatter, set `locale: nl`. The collection schema validates the `locale` field against the `locales` list in `src/config/i18n.config.ts`, so any locale you register there is accepted automatically — there's no separate enum to edit. The base theme is ready for this — the field, folder convention, per-post type-safety, **and the routing** are all in place: drop posts in `src/content/blog/nl/` and the localized blog is generated for you — the index (`/nl/blog`), each post (`/nl/blog/<slug>`), pagination (`/nl/blog/page/N`), and tag archives (`/nl/blog/tag/<tag>`), with every in-locale link staying inside that locale. There are no per-locale blog page files to create.
...
**Projects work exactly the same way.** Drop translations in `src/content/projects/<locale>/` (the bundled projects live in `src/content/projects/en/`) and the localized projects section is generated for you — the index (`/nl/projects`), each project (`/nl/projects/<slug>`), pagination (`/nl/projects/page/N`), and tag archives (`/nl/projects/tag/<tag>`), with every in-locale link staying inside that locale. Projects share one slug across locales — keep the same filename in each locale folder (e.g. `en/studio-portfolio.mdx` ↔ `nl/studio-portfolio.mdx`) and the theme pairs them automatically for `hreflang` and the `LanguageSwitcher`. (Blog posts can instead use locale-specific slugs paired by a canonical `uid`; projects keep it simple with matching filenames.)
...
The `LanguageSwitcher` and the `hreflang` tags link to each translation's **real** URL — so a visitor reading `/blog/hello-world` who clicks "Nederlands" lands on the actual Dutch post, and the theme works out which URL that is:

1. **Keep the slug identical across locales** — `/blog/hello-world` ↔ `/nl/blog/hello-world`, paired automatically with zero config.
2. **Use locale-specific slugs** — more natural for SEO (`/nl/blog/hallo-wereld`). Give both posts the same canonical `uid` in frontmatter (the same id `<PostLink>` uses) and the theme links them correctly across locales and advertises the right URL in `hreflang`.

Either way, a locale with no translation of the current post is dropped from `hreflang`, and the switcher sends the visitor to that locale's blog index instead of a dead URL — so switching language never 404s.
