...
In the Dutch post's frontmatter, set `locale: nl`. The collection schema validates the `locale` field against the `locales` list in `src/config/i18n.config.ts`, so any locale you register there is accepted automatically — there's no separate enum to edit. The base theme is ready for this — the field, folder convention, and per-post type-safety are all in place.
