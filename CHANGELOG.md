...
- **Blog post tags moved directly below the abstract** — on a blog post the tag chips rendered *above* the title; `ArticleHero` now places its `TagList` immediately beneath the intro/abstract paragraph (above the author·date·reading-time meta row), as requested in the issue. Purely presentational. (#423)
