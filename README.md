...
>
> The `LanguageSwitcher` swaps the locale segment of the current path, so cross-locale links assume a post keeps the **same slug** across locales (`/blog/hello` ↔ `/nl/blog/hello`). If a translation uses a different filename, link to it by stable id with `<PostLink>` (see *Durable Internal Links*) or keep the slugs aligned.
